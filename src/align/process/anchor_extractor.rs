...
use crate::align::process::seeds_to_anchors::GroupSeeds;

use super::seeds_to_anchors::SeedsToAnchors;
        ...
        while !indices.is_empty() {
            ...
            for index in indices.iter().skip(1) {
                ...
                if indel_first == 0 && forward.is_none_or(|val| val == fwd) {
                    ...
                    // Breaks if seeds are added with indels
                    other_indices.push(*index);
                    // if indel_first > 0 && indel_first == 0 {
                    //     indel_flag = true;
    ...
    fn generate(
        ...
        while current_idx < self.groups.len() - 1 {
            ...
            if current.reference == next.reference {
                ...
                GroupSeeds::group_into_anchor(&seeds_rev[current.start as usize..current.start as usize + current.size as usize], read_length_rev, &mut self.anchors_rev);
                ...
                GroupSeeds::group_into_anchor(&seeds_fwd[next.start as usize..next.start as usize + next.size as usize], read_length_fwd, &mut self.anchors_fwd);
