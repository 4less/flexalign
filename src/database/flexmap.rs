...
> {
    flexmap: Arc<FlexmapBlob<C, F, CELLS_PER_BODY, HEADER_THRESHOLD>>,
...
>  FlexalignDatabase for DB<K, C, F, S, L, CELLS_PER_BODY, HEADER_THRESHOLD> {
    ...
    fn build(options: &crate::options::Options) -> Self {
        ...
        flexmap.save_blob(&index_path);
        let flexmap_blob = Flexmap::<C, F, CELLS_PER_BODY, HEADER_THRESHOLD>::load_blob(&index_path);

        Self {
            flexmap: Arc::new(flexmap_blob),
            rid_to_rname,
            rname_to_rid,
            references,
        }
    ...
    fn load(paths: &super::common::DBPaths, version: u32) -> Self {
        ...
        let flexmap = Flexmap::<C, F, CELLS_PER_BODY, HEADER_THRESHOLD>::load_blob(&index_path);
    ...
    fn save(&self, paths: &DBPaths, version: u32) -> Result<(), std::io::Error> {
        std::fs::write(&paths.index_path, self.flexmap.backing_bytes())?;
